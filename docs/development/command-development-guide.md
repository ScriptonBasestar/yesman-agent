...
1. **명명 규칙**: `{command_name}.py` 형식으로 파일명 지정
1. **메인 CLI 등록**: `yesman.py`의 메인 CLI에 명령어 추가
...
1. **멱등성**: 같은 입력에 대해 같은 결과를 보장
1. **사용자 경험**: 명확한 메시지와 도움말 제공
1. **에러 복구**: 실패 시 복구 방법 안내
1. **테스트 커버리지**: 모든 주요 코드 경로에 대한 테스트
...
**Q: 의존성 주입이 작동하지 않음** A: `initialize_services()`가 호출되었는지 확인하고, DI 컨테이너에 서비스가 등록되었는지 확인하세요.

**Q: 테스트에서 명령어가 실제 실행됨** A: `patch.object(Command, '__init__', lambda x: None)`을 사용하여 초기화를 모킹하세요.

**Q: 비동기 명령어에서 에러 발생** A: 모든 외부 의존성이 올바르게 모킹되었는지 확인하고, `AsyncMonitoringCommand`를 상속받았는지 확인하세요.
